...
class Biom:
    ...
    def __init__( self, id=None, format="Biological Observation Matrix 1.0.0", 
                  ...
                  matrix_type="dense", matrix_element_type="int", comment=None, 
                  data=None ):
        """
        @param id: [int]
        @param format: [str]
    ...
    def add_group_order( self, metadata_file):
        """
        @summary: Add a comment with the sample group order as from in the input metadata file.
        @param metadata_file : [str] The path of the sample metadata file.
        """
        ...
        if subject_type not in ["sample"]:
            raise Exception( "The subject type in first line of file '" + metadata_file + "' must be 'sample' when add_group_order() is called." )
        
        group_tab = []
        group_tab.append(title_fields)
        for line in metadata_fh:
            if not line.startswith('#'):
                line_fields = [field.strip() for field in line.split("\t")]
                group_tab.append(line_fields)
        
        metadata_fh.close()
        
        self.comment = ""
        for j in range(1,len(title_fields)):
            uniq_group_tab = []
            group_tag = group_tab[0][j]
            
            for i in range(1,len(group_tab)):
                if group_tab[i][j] not in uniq_group_tab:
                    uniq_group_tab.append(group_tab[i][j])
            self.comment += "__" + group_tag + "_group_order=" + ";".join(uniq_group_tab) + "__ "
...
class BiomIO:
    ...
    def from_json( path ):
        ...
        if "comment" not in python_dict: python_dict["comment"] = None
    ...
    @staticmethod
    def load_metadata( biom, metadata_file, types=None, list_sep=None, subject_type=None, group_order=False ):
        """
		...
		@param group_order : [bool] Flag to record the group order from the metadata input file into the biom (ONLY FOR subject_type="sample")
		"""
		...
		# Add sample group order in the biom comment if group_order is true
        if "sample" == subject_type and group_order:
            biom.add_group_order(metadata_file)
